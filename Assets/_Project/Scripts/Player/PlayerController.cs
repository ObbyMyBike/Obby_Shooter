...
#endif
        _fireSubscription = _input.Fire.ThrottleFirst(TimeSpan.FromSeconds(1f / _settings.FireRate)).Subscribe(_ => Fire());
        
        // _moveSubscription = _input.MoveAxis.Subscribe(direction =>
        //     {
        //         Vector3 worldDirection = transform.TransformDirection(new Vector3(direction.x, 0f, direction.y));
        //         _characterController.Move(worldDirection * (_settings.SpeedMovement * Time.deltaTime));
        //     });
        // _fireSubscription = _input.Fire.ThrottleFirst(TimeSpan.FromSeconds(1f / _settings.FireRate)).Subscribe(_ => Fire());
    }
